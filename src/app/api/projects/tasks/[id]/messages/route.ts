...
export async function GET(
  ...
  if (!canAccessTaskChat(auth.user.id, auth.user.role, auth.user.masterDataScope, dto)) {
...
export async function POST(
  ...
  try {
    ...
    if (!canAccessTaskChat(auth.user.id, auth.user.role, auth.user.masterDataScope, access)) {
      return NextResponse.json({ error: "Not allowed" }, { status: 403 });
    }
